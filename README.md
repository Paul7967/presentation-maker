...
2. Для каждого слайда можно указать **тип шаблона (layoutId)** — индекс (0, 1, …) или имя layout из образца PPTX; при пустом значении используется первый шаблон. Также можно ввести **заметки докладчика** (до 4000 символов) — они попадут в speaker notes итогового PPTX.
3. При необходимости загрузите готовое описание из файла .json — перетащите файл в зону «Загрузить из JSON» или выберите его по клику.
4. Выберите файл-образец в формате .pptx (до 20 MB).
5. Нажмите «Сгенерировать».
6. После завершения нажмите «Скачать» для сохранения PPTX или «Создать ещё» для новой презентации.

### Режим «Через JSON»

1. Переключитесь на «Через JSON» в шапке — в поле подставится текущее описание слайдов в формате JSON.
2. Редактируйте JSON вручную (формат: массив объектов с полями `title`, `items`, опционально `layoutId` (число или строка), `notes` (строка, до 4000 символов) — см. подсказку под полем).
...
Лимиты: не более 100 слайдов; заголовок — до 500 символов, пункт — до 2000 символов, до 50 пунктов на слайд; заметки докладчика — до 4000 символов на слайд (см. [docs/api.md](docs/api.md)). Файл .json при загрузке ограничен 2 MB.
